...
public:
  ...
  : Node("mission_executive", options)
  {
    ...
    // TF — used to look up map→base_link for arrival and cross-track checks
    ...
    // IMU-based stop detection: use angular velocity magnitude from the gyro.
    // A stopped rover has near-zero rotation rate; stop_angular_vel_threshold (rad/s)
    // controls how small it must be to count as "stopped".
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      imu_topic, rclcpp::SensorDataQoS(),
      [this](const sensor_msgs::msg::Imu::SharedPtr msg) {
        std::lock_guard<std::mutex> lk(mutex_);
        const auto & w = msg->angular_velocity;
        imu_angular_vel_ = std::sqrt(w.x * w.x + w.y * w.y + w.z * w.z);
        checkStopDetection();
      });
...
private:
  ...
  void publishStatus()
  {
    ...
    s.robot_speed_mps     = imu_angular_vel_;  // IMU angular velocity magnitude (rad/s)
  ...
  // Look up map→base_link and cache the result in robot_pose_.
  // Called from the status timer; silently does nothing if the TF is not yet available.
  void refreshPoseFromTF()
  {
    try {
      ...
      // TF not yet available — robot_pose_ stays as-is (nullopt at startup)
    }
  }
  ...
  void checkStopDetection()
  {
    ...
    if (imu_angular_vel_ < stop_angular_vel_threshold_) {
  ...
  double                                          imu_angular_vel_{0.0};  // rad/s
