  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // angular.z = desired turning rate (rad/s), which we convert to steering angle
  double linear_vel_cmd = (*current_ref)->twist.linear.x;

  // Convert angular velocity to steering angle using wheelbase
  // For Ackermann steering: angular_vel = linear_vel * tan(steer_angle) / wheelbase
  // Therefore: steer_angle = atan(angular_vel * wheelbase / linear_vel)
