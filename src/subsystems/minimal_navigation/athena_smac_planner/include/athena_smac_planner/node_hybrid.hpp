...
#ifndef ATHENA_SMAC_PLANNER__NODE_HYBRID_HPP_
...
#include "athena_smac_planner/obstacle_heuristic.hpp"
#include "athena_smac_planner/distance_heuristic.hpp"
...
/**
 * @struct athena_smac_planner::HybridMotionTable
 * @brief A table of motion primitives and related functions
 */
struct HybridMotionTable
{
  /**
   * @brief A constructor for athena_smac_planner::HybridMotionTable
   */
  HybridMotionTable() {}
...
/**
 * @class athena_smac_planner::NodeHybrid
...
public:
  ...
  using Coordinates = athena_smac_planner::Coordinates;
  typedef std::vector<Coordinates> CoordinateVector;

  struct NodeContext
  {
    /**
     * @brief A constructor for athena_smac_planner::NodeContext
     */
    NodeContext()
    {
  ...
  /**
   * @brief A constructor for athena_smac_planner::NodeHybrid
   * @param index The index of this node for self-reference
   */
  explicit NodeHybrid(const uint64_t index, NodeContext * ctx);

  /**
   * @brief A destructor for athena_smac_planner::NodeHybrid
   */
  ~NodeHybrid();
  ...
  void getNeighbors(
    ...
    athena_smac_planner::NodeHybrid * &)> & validity_checker,
    GridCollisionChecker * collision_checker,
    const bool & traverse_unknown,
...
#endif  // ATHENA_SMAC_PLANNER__NODE_HYBRID_HPP_
