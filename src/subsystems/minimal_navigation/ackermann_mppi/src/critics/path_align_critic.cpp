...
void PathAlignCritic::initialize()
{
  ...
  RCLCPP_INFO(
    logger_, "PathAlignCritic instantiated with %d power and %f weight",
    power_, weight_);
}

void PathAlignCritic::score(CriticData & data)
{
  if (!enabled_ || data.state.local_path_length < threshold_to_consider_) {return;}
  ...
  if (path_segments_count < offset_from_furthest_) {return;}
  ...
  int strided_traj_cols =
    static_cast<int>(floor((data.trajectories.x.cols() - 1) / trajectory_point_step_)) + 1;
  ...
  unsigned int num_samples = 0u, path_pt = 0u;
  ...
  for (size_t t = 0; t < batch_size; ++t) {
    ...
    cost(t) = num_samples > 0u ?
      summed_path_dist / static_cast<float>(num_samples) : 0.0f;
  }
